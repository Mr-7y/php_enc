...
/* Modify enc_key to set the secret key */
const u_char enc_key[] = {
        0x9f, 0x49, 0x52, 0x00,
        0x58, 0x9f, 0xff, 0x23,
        0x8e, 0xfe, 0xea, 0xfa,
        0xa6, 0x33, 0xf3, 0xc6,
...
int enc_ext_fopen(FILE *, struct stat *, ENC_RES *, const char *);
...
zend_op_array *cgi_compile_file(zend_file_handle *file_handle, int type)
{
    ...
    if (data_len >= sizeof(enc_header)) {
        char *t = emalloc(sizeof(enc_header));
        size_t read_cnt = fread(t, sizeof(enc_header), 1, fp);
        /* If not the encoded file */
        if (memcmp(t, enc_header, sizeof(enc_header))) {
            efree(t);
            fclose(fp);
            goto final;
        }
...
int enc_ext_fopen(FILE *fp, struct stat *stat_buf, ENC_RES *res, const char *file_name)
{
    char *p_data;
    size_t data_len;
    size_t write_len;
    ...
    if (tmpfile_s(res)) {
        php_error_docref(NULL, E_CORE_ERROR, "enc: Failed to create temp file, may be too many open files.\n");
        efree(p_data);
        return -1;
    }

    if (fwrite(p_data, data_len, 1, *res) != 1) {
        php_error_docref(NULL, E_CORE_ERROR, "enc: Failed to write temp file.\n");
        efree(p_data);
        fclose(*res);
        return -2;
    }
...
void enc_encode(char *data, size_t len)
{
    size_t i, p = 0;
    for (i = 0; i < len; ++i) {
        if (i & 1) {
...
void enc_decode(char *data, size_t len)
{
    size_t i, p = 0;
    for (i = 0; i < len; ++i) {
        if (i & 1) {
